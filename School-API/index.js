...
// --------------------
// Schemas + Models (TWO collections)
// --------------------
...
app.post("/tests", async (req, res) => {
  ...
  await Course.updateOne({ id: cId }, { $push: { tests: newTest } });
...
app.get("/students/:id/average", async (req, res) => {
  ...
  const avg =
    studentTests.reduce((acc, x) => acc + (x.mark / x.outOf) * 100, 0) / studentTests.length;
